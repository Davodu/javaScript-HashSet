...
HashSet.prototype.addAll = function(r){
	var k = r.values();	 
	for(var i=0;i<k.length; i++){ this.add(k[i]);} 
}

HashSet.prototype.removeAll = function(r){
	var k = r.values();
	for(var i=0;i<k.length; i++){ this.remove(k[i]);} 
	return this;
}
...
HashSet.prototype.union = function(r){
	...
	var k = r.values();	 
	var arr = this._arr;
    for(var i=0;i<arr.length; i++){ x.add(arr[i]);} 
    for(var i=0;i<k.length; i++){ x.add(k[i]);} 
    return x;
}
...
HashSet.prototype.intersection = function(r){    
    ...
    for(var i=0;i<k.length; i++){
    	if(this.contains(k[i])){x.add(k[i]);}
    }
    return x;
}
...
HashSet.prototype.isSubsetOf = function(r){
	...
	for(var i=0; i<arr.length; i++ ){
		if(!r.contains(arr[i])){return false;}
	}
	return true;
}
...
HashSet.prototype.complement = function(r){
	...
	var arr = this._arr;
	for(var i=0; i<arr.length; i++ ){
		if(!r.contains(arr[i])){x.add(arr[i])}
	}
	return x;
}
